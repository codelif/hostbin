...
func TestVerifierRejectsReplayButAllowsValidRetryAfterInvalidSignature(t *testing.T) {
	...
	invalidReq.Header.Set(HeaderSignature, strings.Repeat("0", 64))
...
func testEngine(t *testing.T) (*gin.Engine, time.Time) {
	...
	engine.Use(httpserver.LimitBodyBytes(1024), verifier.Middleware())
...
func signedRequest(t *testing.T, method, path string, body []byte, timestamp time.Time, nonceValue string) *http.Request {
	...
	canonical := CanonicalRequest(req, SHA256Hex(body), req.Header.Get(HeaderTimestamp), nonceValue)
	req.Header.Set(HeaderSignature, hex.EncodeToString(Sign([]byte("01234567890123456789012345678901"), canonical)))
